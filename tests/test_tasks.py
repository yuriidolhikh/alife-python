...
async def test_combat_task(monkeypatch):
    ...
    squad1 = Squad("test_faction1", (1, 1))
    squad1.add_actor(Actor("test_faction1", (1, 1)))

    squad2 = Squad("test_faction2", (1, 1))
    squad2.add_actor(Actor("test_faction2", (1, 1)))
    ...
    assert not squad2.actors, "squad2 should not have alive actors"
    assert not squad2.in_combat, "squad2 should not be in combat"

    squads, actors = grid.get_grid()[(1, 1)]

    assert not squads, "Actorless squads should not be on the map"
